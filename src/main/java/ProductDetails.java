	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
           ...
           // props.load(in);
            ...
            DBConnection conn = new DBConnection("jdbc:mysql://localhost:3306/ecommerce", "root", "T@nmay8399");
            ...
            ResultSet rst = stmt.executeQuery("select * from pets.products");
            ...
            {	
                ...
                while (rst.next()) {
                        out.println(rst.getInt("ID") + ": " + rst.getString("name") + " " 
                        		+ rst.getString("color") + " costs: $" + rst.getDouble("price") + "<Br>");
                }
            }
            ...
            else 
            {
            	...
            	String sql_res= "select * from pets.products where id=" + productSearch;
                ResultSet inTable = stmt.executeQuery(sql_res);
                
                //if not empty then print all product details
                if(inTable.next())
                	out.println(inTable.getInt("ID") + ": " + inTable.getString("name") + " " 
                		+ inTable.getString("color") + " costs: $" + inTable.getDouble("price") + "<Br>");
